...
export default function EventModal ({pickedEvent,streetAddress,onClose,setCurrWeekEvents, setEvents,events,currWeekEvents,athletes,auxiliaryStatus,fullAddress}){
    ...
    const handleAcceptRequest = async() => {
        ...
        // Safety check for reminders during acceptance
        if (pickedEvent?.reminder?.quantity) {
        ...
        sendMessage(pickedEvent.contact[0].phoneNumber, message)
    }
    ...
    useEffect(()=>{
        const getSeries = async()=>{
            ...
            if (retrieveSeriesInfo && retrieveSeriesInfo[0]) {
    ...
    return(
        <>
            <div className="p-[8px]" style={{ userSelect: "none", WebkitUserSelect: "none" }} >
                {(!isDeleting && pickedEvent?.status?.toLowerCase() == "available") && (
                    <div className="absolute top-[10px] h-[20px] right-[42px] cursor-pointer" >
                    ...
                    <div>
                        {pickedEvent.seriesId ? (
                            <div>
                                {seriesInfo ? (
                                    <>
                                        <div className="text-[14px] text-center">This event belongs to the following weekly series:</div>
                                        <div className="flex justify-center text-[14px] space-x-[5px] text-center mt-[10px] mb-[5px]">
                                            <div className="text-[14px] font-bold text-center">Days:</div>
                                            <div className="text-[14px] text-center">
                                                {seriesInfo.days?.map((day) => dayMap[day.toLowerCase()] || day).join(" - ")}
                                            </div>
                                        </div>
                                        <div className="text-[14px] flex justify-center space-x-[30px] text-center">
                                            <div className="flex justify-center space-x-[5px] text-[14px] text-center">
                                                <div className="text-[14px] font-bold text-center">Start Date:</div>
                                                <div className="text-[14px] text-center">{seriesInfo.dateStart?.toDateString()}</div>
                                            </div>
                                            <div className="flex justify-center text-[14px] space-x-[5px] text-center">
                                                <div className="text-[14px] font-bold text-center">End Date:</div>
                                                <div className="text-[14px] text-center">{seriesInfo.dateEnd?.toDateString()}</div>
                                            </div>
                                        </div>
                                        <div className="flex mt-[12px] justify-center items-center space-x-[30px] w-full">
                                            <div className={`py-[6px] text-[14px] px-[10px] text-white font-bold bg-red-500 rounded-full cursor-pointer ${hasClickedSubmit?"opacity-50":""}`} onClick={handleDeleteSeries}>Delete entire series</div>
                                            <div className={`py-[6px] text-[14px] px-[10px] text-white font-bold bg-red-500 rounded-full cursor-pointer ${hasClickedSubmit?"opacity-50":""}`} onClick={handleDeleteEvent}>Delete only this event</div>
                                            <div className="font-bold text-[14px] text-streamlineBlue cursor-pointer" onClick={()=>{setIsDeleting(false)}}>Cancel</div>
                                        </div>
                                    </>
                                ) : <div className="w-full flex justify-center"><LoadingSubScreen/></div>}
                            </div>
                        ) : (
                            <div>
                                <div>Are you sure you want to delete this event?</div>
                                <div className="flex mt-[12px] justify-center items-center space-x-[50px] w-full">
                                    <div className={`py-[6px] px-[10px] text-white font-bold bg-red-500 rounded-full cursor-pointer ${hasClickedSubmit?"opacity-50":""}`} onClick={handleDeleteEvent}>Yes, delete this event</div>
                                    <div className="font-bold text-streamlineBlue cursor-pointer" onClick={()=>{setIsDeleting(false)}}>No</div>
                                </div>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="relative">
                        <div className="flex flex-col">
                            <div className="flex items-center space-x-[16px] mt-[10px]">
                                <div className="flex w-[22px] justify-center items-center">
                                    <div className="flex w-[14px] h-[14px] rounded-[4px]" style={{backgroundColor:CONFIG.calendar.blockColors[auxiliaryStatus?auxiliaryStatus.toLowerCase():pickedEvent?.status?.toLowerCase()] || "#ccc"}}/>
                                </div>
                                <div className="flex font-bold text-[18px]">{pickedEvent.title}</div>
                            </div>
                        </div>
                        ...
                        <div className="flex flex-col pt-[8px]">
                            <div className="flex items-center space-x-[16px]">
                                <div className="flex w-[22px] mt-[5px] justify-center items-center"><InfoIcon/></div>
                                <div className="flex flex-col">
                                    <div className="flex leading-[14px] text-[14px] mt-[5px]">
                                        <div className="font-bold mr-[4px]">Status:</div>    
                                        <div>{auxiliaryStatus ? auxiliaryStatus : pickedEvent.status}</div>    
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        {(selectedLessonTypes && (userInfo.teamInfo || userInfo.userData == null)) && (
                            <>
                                {(pickedEvent.numberOfSpots || pickedEvent.numberOfAthletes) && (
                                    <div className="flex text-[14px] ml-[38px]">
                                        {pickedEvent.numberOfSpots ? (
                                            <>
                                                <div className="font-bold mr-[4px]">Spots:</div>    
                                                <div>{pickedEvent.numberOfSpots}</div>    
                                            </>
                                        ) : (
                                            <>
                                                <div className="font-bold mr-[4px]">Attendees:</div>
                                                <div className="mr-[4px]">{pickedEvent.numberOfAthletes} {CONFIG.athleteType}{pickedEvent.numberOfAthletes>1?"s":""}</div>    
                                            </>
                                        )}
                                    </div>
                                )}
                                <div className="text-[14px] leading-[8px] mt-[5px] mb-[3px] ml-[38px] font-bold">{pickedEvent.status=="Available"?"Applicable lesson types:":"Lesson type"}</div>
                                <div className="text-[14px] ml-[38px] ">{selectedLessonTypes?.join(", ") || "N/A"}</div>
                                <div className="text-[14px] leading-[8px] mt-[8px] mb-[3px] ml-[38px] font-bold">{pickedEvent.status=="Available"?"Applicable skill levels:":"Skill level"}</div>
                                <div className="text-[14px] ml-[38px] ">{selectedSkillsLevels?.join(", ") || "N/A"}</div>
                            </>
                        )}
    ...
    {locationData?.address || streetAddress || "No address available"}
                        ...
                        <div className="flex items-center py-[12px] space-x-[15px] mt-[3px]">
                            <div className="flex w-[22px] justify-center items-center"><NotifIcon/></div>
                            <div className="flex text-[14px]">
                                {/* THE FIX: Optional chaining on reminder */}
                                {pickedEvent?.reminder?.quantity 
                        ...
                        {(pickedEvent.coachName || athletes) && (
                            <>
                                <div className="flex items-center pt-[10px] space-x-[16px]">
                                    <div className="flex w-[22px] justify-center items-center"><PeopleIcon/></div>
                                    <div className="flex font-bold text-[14px]">{pickedEvent.coachName?"Coach":CONFIG.athleteType}</div>
                                </div>
                                <div className="ml-[7px] space-y-[14px]">
                                    {pickedEvent.coachName && <PersonEntry personInfo={{fullName:pickedEvent.coachName, email:pickedEvent.coachEmail, phoneNumber:pickedEvent.coachPhone}}/>}
                                    {athletes?.length > 0 && (
                                        <>
                        ...
                        {(pickedEvent?.status?.toLowerCase() == "pending" && (userInfo.teamInfo || userInfo.userData == null) && !auxiliaryStatus) && (
                            <div className="flex space-y-[12px] flex-col w-full justify-center items-center mt-[25px]">
                                {(!rejectedStepOne && !acceptedStepOne) ? (
                                    <>
                                        <div className="text-white font-bold py-[8px] text-center w-[240px] cursor-pointer text-[14px] rounded-full" style={{backgroundColor:CONFIG.calendar.blockColors.confirmed}} onClick={() => setAcceptedStepOne(true)}>Accept trial lesson request</div>
                                        <div className="text-white font-bold py-[8px] text-center w-[240px] cursor-pointer text-[14px] rounded-full bg-red-500" onClick={() => setRejectedStepOne(true)}>Reject trial lesson request</div>        
                                    </>
                                ) : (
                                    rejectedStepOne ? (
                                        <div className="flex flex-col items-center justify-center text-[14px] w-[270px] mb-[20px]">
                                            <div className="flex leading-[18px] mb-[16px] text-center">Are you sure you want to reject this trial lesson reservation? The {CONFIG.athleteType.toLowerCase()} will be notified of your response.</div>
                                            <div className="flex justify-center items-center space-x-[12px] w-full px-[12px]">
                                                <div className="py-[6px] px-[10px] text-white font-bold rounded-full cursor-pointer bg-red-500" onClick={handleRejectRequest}>Yes, reject trial lesson</div>
                                                <div className="font-bold px-[10px] text-streamlineBlue cursor-pointer" onClick={() => setRejectedStepOne(false)}>No</div>
                                            </div>
                                        </div>
                                    ) : (
                                        <div className="flex flex-col items-center justify-center text-[14px] mb-[20px]" style={{width:'280px'}}>
                                            <div className="flex leading-[18px] mb-[16px] text-center">Are you sure you want to accept this trial lesson reservation? The {CONFIG.athleteType.toLowerCase()} will be notified of your response.</div>
                                            <div className="flex justify-center items-center space-x-[62px] w-full">
                                                <div className={`py-[6px] px-[10px] text-white font-bold bg-green-500 rounded-full cursor-pointer ${hasClickedSubmit?"opacity-50":""}`} onClick={handleAcceptRequest}>Yes, accept trial lesson</div>
                                                <div className="font-bold text-streamlineBlue cursor-pointer" onClick={() => setAcceptedStepOne(false)}>No</div>
                                            </div>
                                        </div>
                                    )
                                )}
